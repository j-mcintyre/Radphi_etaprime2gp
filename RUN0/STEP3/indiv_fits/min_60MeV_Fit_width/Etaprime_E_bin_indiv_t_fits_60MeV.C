...
void Etaprime_E_bin_indiv_t_fits_60MeV() {

   // Create a Root file to later fill with new histograms for reference in further analysis work
   TFile *StarterFile = new TFile("TwoG_may_16_2023_STEP3_60MeV.root", "NEW");
   StarterFile->Close();

   // File from STEP2 containing information for calculations & fitting
   TFile *fStep2 = new TFile("/nfs/direct/annex/mcintyre/GitRepo/Radphi_etaprime2gp/RUN0/STEP2/total_fit/min_60MeV_Fit_width/TwoG_may_16_2023_STEP2_60MeV.root");
   ...
   // once in 2g_Etaprime_Results_60MeV.txt
   int reset = 0;
   ...
   for (int Echannel = 0; Echannel < NumEChan; Echannel++) {
      ...
      for (int tchannel = 0; tchannel < tBinNum; tchannel++) {
         ...
         */
         ...
         CheckYield.open("Etaprime_yield_check_60MeV.txt", std::ios_base::app);
         ...
         FitYield.open("Etaprime_yield_fit_width_60MeV_min.txt", std::ios_base::app);
         if (FitYield.is_open()) {
         ...
         */
         ...
         FitErrorCalc.open("Etap_fit_status_60MeV.txt", std::ios_base::app);
         ...
         Calcstatus1.open("Etap_Calculations_Check_60MeV.txt", std::ios_base::app);
         ...
         */
         ...
         Fitstatus1.open("Etap_fit_status_60MeV.txt", std::ios_base::app);
         ...
         reset += 1;
			ofstream output;
			output.open("2g_Etap_Results_60MeV.txt", std::ios_base::app);
			if (output.is_open()) {
            if (reset == 1){
               output << "//-------------------------------//" << "\n";
      ...
      TFile *MyFile = new TFile("TwoG_may_16_2023_STEP3_60MeV.root", "UPDATE");
   ...
   // Root file with new histograms for reference in later analysis work
   TFile *yieldFile = new TFile("TwoG_may_16_2023_STEP3_60MeV.root", "UPDATE");
   ...
   if (conv_fit == 1) {
      ...
      gSystem->Exec("mv *Results_60MeV.txt FitSummary/.");
      gSystem->Exec("mv *fit_status_60MeV.txt FitSummary/.");

      gSystem->Exec("mv Etap_Calculations_Check_60MeV.txt calc_check/.");
      gSystem->Exec("mv Etaprime_yield_check_60MeV.txt calc_check/.");
      gSystem->Exec("mv Errors_60MeV.txt calc_check/.");
      gSystem->Exec("mv Etaprime_yield_fit_width_60MeV_min.txt calc_check/.");
   }
   ...
   // Failed Fitting
      ...
      gSystem->Exec("mv *Results_60MeV.txt FAIL/.");
      gSystem->Exec("mv *fit_status_60MeV.txt FAIL/TRASH/.");
