...
//    -----------------------------------------
//    PROOF Service Used --> 1097 
//                           /* Jim's 1099    */
//                           /* Fridah's 1097 */
//    Radphi Dataset     --> pass-9-2020
//    This files name  --> proof_TwoG_real2_etaprime2gp_may_16_2023
//    TSelector Name   --> TwoG_real2_etaprime2gp_may_16_2023
//
//   ///////////////// For MC Data ///////////////////
...
// --> TwoG_real2_etaprime2gp_may_16_2023Analysis.root
// --------------------------------------------------------
...
TString proofserver("proof://stat31.phys.uconn.edu:1097/?N");
...
TChain *fullchain()
{
   ...
   while (const char *fname = gSystem->GetDirEntry(datadir)) {
      ...
      //if ((fnames.find("etaprime2gp_gen") != fnames.npos) && (fnames.find(".root") != fnames.npos)) {  // For MC data
   ...
   //std::cout << "There are ==> " << filecount << " Monte Carlo files with name(s) - " << "etaprime2gp_gen" << " - being processed. \n" ;
   gSystem->FreeDirectory(datadir);
   return ch;
}


void proof_TwoG_real2_etaprime2gp_may_16_2023(int nevents=2000000000)
{
   TChain *ch=fullchain();
   ...
   if (gProof == 0) {
      ...
      gProof->SetParameter("PROOF_MaxSlavesPerNode", (Int_t)108);
   }

   gProof->Load("TwoG_real2_etaprime2gp_may_16_2023.C+O");
   ch->SetProof();
#else
   gROOT->ProcessLine(".L TwoG_real2_etaprime2gp_may_16_2023.C+O");
#endif

   ch->Process("TwoG_real2_etaprime2gp_may_16_2023", "", nevents);
